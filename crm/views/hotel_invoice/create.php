...
<div class="text-end"><strong class="text-success"><?php echo number_format($inv->final_amount); ?> ت</strong><br><span class="badge <?php echo ($inv->invoice_status=='settled'||$inv->invoice_status=='paid')?'bg-success':($inv->invoice_status=='prepaid'?'bg-info':'bg-warning text-dark'); ?>" style="font-size:9px;"><?php echo $inv->invoice_status=='paid'?'پرداخت':($inv->invoice_status=='settled'?'تسویه':($inv->invoice_status=='prepaid'?'پیش':'مانده')); ?></span></div>
                        </div>
