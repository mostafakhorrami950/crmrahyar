...
<div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-3">
    <h5 class="fw-bold mb-0">🗺️ نگاشت ستون‌ها — <?php echo htmlspecialchars($fileName); ?></h5>
    <a href="<?php echo $config['url']; ?>/contacts/import" class="btn btn-outline-secondary">بازگشت</a>
</div>

<!-- Progress Steps -->
<div style="display:flex;gap:8px;margin-bottom:20px;justify-content:center;">
    <div style="background:#d4edda;color:#155724;padding:8px 16px;border-radius:20px;font-size:13px;font-weight:600;"><i class="bi bi-check-circle text-success me-1"></i> ۱. آپلود فایل</div>
    ...
    <div style="color:var(--gray-400);padding:8px 4px;">→</div>
    <div style="background:var(--gray-200);color:var(--gray-600);padding:8px 16px;border-radius:20px;font-size:13px;"><i class="bi bi-check-circle text-success me-1"></i> ۳. تایید و ایمپورت</div>
</div>

<div class="card">
    <p style="font-size:13px;color:var(--gray-600);margin-bottom:16px;">
        <i class="bi bi-bar-chart me-1"></i> فایل <strong><?php echo htmlspecialchars($fileName); ?></strong> — 
    ...
    <form id="mappingForm">
            ...
            <table>
                <thead>
                    <tr>
                        ...
                        <th class="text-nowrap">عنوان ستون در فایل</th>
                        <th style="min-width:200px;">← نگاشت به فیلد</th>
                        <th class="text-nowrap">نمونه داده (ردیف ۱)</th>
                        <th class="text-nowrap">نمونه داده (ردیف ۲)</th>
                    </tr>
                </thead>
                <tbody>
        ...
        <details style="margin-top:12px;">
            <summary style="cursor:pointer;font-size:12px;color:var(--gray-500);"><i class="bi bi-search me-1"></i> نمایش نمونه‌های بیشتر...</summary>
...
<div class="card" style="margin-top:12px;">
    <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:12px;">
        ...
        <div style="display:flex;gap:8px;">
            <button type="button" class="btn btn-outline-secondary" onclick="doPreview()">👁️ پیش‌نمایش</button>
            <button type="button" class="btn btn-primary btn-lg" onclick="doImport()" id="importBtn"><i class="bi bi-check-circle text-success me-1"></i> تایید و ایمپورت <?php echo number_format($totalRows); ?> ردیف</button>
        </div>
    </div>
</div>
...
<div id="previewModal" style="display:none;position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.5);z-index:1000;overflow-y:auto;padding:20px;">
    <div style="max-width:900px;margin:0 auto;background:#fff;border-radius:16px;padding:24px;max-height:90vh;overflow-y:auto;">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;">
            <h5 class="fw-bold mb-0">👁️ پیش‌نمایش ایمپورت</h5>
            <button onclick="closePreview()" style="background:none;border:none;font-size:20px;cursor:pointer;">✕</button>
        </div>
        <div id="previewContent"></div>
        <div style="display:flex;gap:8px;margin-top:16px;justify-content:flex-end;">
            <button class="btn btn-outline-secondary" onclick="closePreview()">بازگشت</button>
            <button class="btn btn-primary btn-lg" onclick="doImport(); closePreview();"><i class="bi bi-check-circle text-success me-1"></i> تایید و ایمپورت</button>
        </div>
    </div>
</div>
...
<div id="resultModal" style="display:none;position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.5);z-index:1000;overflow-y:auto;padding:20px;">
    <div style="max-width:500px;margin:100px auto;background:#fff;border-radius:16px;padding:24px;text-align:center;">
        <div id="resultIcon" style="font-size:64px;margin-bottom:16px;"><i class="bi bi-clock text-warning me-1"></i></div>
        <h5 class="fw-bold mb-0">در حال ایمپورت...</h5>
...
function showPreview(preview, total) {
    var html = '<p style="font-size:13px;color:var(--gray-600);margin-bottom:12px;"><i class="bi bi-bar-chart me-1"></i> ' + total + ' ردیف در فایل — نمایش ۱۰ ردیف اول:</p>';
    ...
    for (var k in getMapping()) {
        html += '<th class="text-nowrap">' + (fieldLabels[getMapping()[k]] || getMapping()[k]) + '</th>';
    }
    html += '</tr></thead><tbody>';
...
function doImport() {
    ...
    btn.textContent = '<i class="bi bi-clock text-warning me-1"></i> در حال ایمپورت...';
    btn.disabled = true;

    // Show loading modal
    document.getElementById('resultIcon').textContent = '<i class="bi bi-clock text-warning me-1"></i>';
    ...
    fetch(baseUrl + '/contacts/import/execute', { 
        ...
        .then(function(data) {
            if (data.success) {
                document.getElementById('resultIcon').textContent = '<i class="bi bi-check-circle text-success me-1"></i>';
                document.getElementById('resultTitle').textContent = 'ایمپورت با موفقیت انجام شد!';
                document.getElementById('resultMessage').textContent = data.message;
                
                var details = '<strong><i class="bi bi-check-circle text-success me-1"></i> ایمپورت شده:</strong> ' + data.imported + '<br>';
                if (data.skipped > 0) details += '<strong>⏭️ نادیده گرفته شده:</strong> ' + data.skipped + '<br>';
                if (data.errors && data.errors.length) {
                    details += '<strong><i class="bi bi-x-circle text-danger me-1"></i> خطاها:</strong><br>';
                    data.errors.forEach(function(e) { details += '• ' + e + '<br>'; });
                }
                document.getElementById('resultDetails').innerHTML = details;
            } else {
                document.getElementById('resultIcon').textContent = '<i class="bi bi-x-circle text-danger me-1"></i>';
