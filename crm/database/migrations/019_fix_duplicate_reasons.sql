...
-- Add UNIQUE key if not exists (will silently fail if already exists)
ALTER TABLE deal_win_reasons ADD UNIQUE KEY IF NOT EXISTS unique_win_reason_name (name);
...
ALTER TABLE deal_loss_reasons ADD UNIQUE KEY IF NOT EXISTS unique_loss_reason_name (name);